...
def inspect_fixture(sqlite_path: Path, duckdb_path: Path) -> dict[str, object]:
    ...
    with sqlite3.connect(sqlite_path) as connection:
        ...
        exploration_tables = [
            str(row[0])
            for row in connection.execute(
                "SELECT name FROM sqlite_master WHERE type = 'table' AND name IN ('exploration_threads', 'exploration_anchors', 'exploration_board_items') ORDER BY name"
            ).fetchall()
        ]
        exploration_indexes = [
            str(row[0])
            for row in connection.execute(
                "SELECT name FROM sqlite_master WHERE type = 'index' AND name LIKE 'idx_exploration_%' ORDER BY name"
            ).fetchall()
        ]
        research_tables = [
            str(row[0])
            for row in connection.execute(
                "SELECT name FROM sqlite_master WHERE type = 'table' AND name IN ('research_runs', 'research_plan_revisions', 'research_observations', 'research_run_events') ORDER BY name"
            ).fetchall()
        ]
        research_indexes = [
            str(row[0])
            for row in connection.execute(
                "SELECT name FROM sqlite_master WHERE type = 'index' AND name LIKE 'idx_research_%' ORDER BY name"
            ).fetchall()
        ]
        analysis_snapshot_tables = [
            str(row[0])
            for row in connection.execute(
                "SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'analysis_snapshots' ORDER BY name"
            ).fetchall()
        ]
        analysis_snapshot_indexes = [
            str(row[0])
            for row in connection.execute(
    ...
    return {
        ...
        "explorationTables": exploration_tables,
        "explorationIndexes": exploration_indexes,
