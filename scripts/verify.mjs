...
const verifyDataDir = mkdtempSync(join(tmpdir(), "aibi-hybrid-verify-"));
...
const ignoredDirs = new Set([".git", "node_modules", "dist", "data", "tmp", "logs"]);
...
checks.push(
  ...
  },
  {
    ...
    ok: verifySource.includes("function compactReceipt(receipt)") &&
      verifySource.includes("const fullOutput = process.argv.includes(\"--full\") || process.env.AIBI_VERIFY_FULL === \"1\"") &&
      verifySource.includes("fullReceiptPath: verifyReceiptPath") &&
      verifySource.includes("writeFileSync(verifyReceiptPath") &&
      verifySource.includes("fullOutput ? receipt : compactReceipt(receipt)"),
  },
  {
