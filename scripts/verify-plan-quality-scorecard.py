...
with tempfile.TemporaryDirectory(prefix="aibi-plan-quality-", ignore_cleanup_errors=True) as temp_value:
    ...
    check("schema-v18-plan-quality-store-boots", status.get("ok") is True and pragma_version == 18 and "plan_quality_scorecards" in tables, {"status": status, "pragma": pragma_version})
