...
function assertPageState() {
  ...
  return {
    ...
    noSampleCopy: !/样例|示例|demo|test/i.test(text),
    home: {
...
try {
  ...
  checks.push(
    ...
    check("api-has-real-tables", Number(counts.tables ?? 0) > 0, { tables: counts.tables }),
    check("api-has-source-intelligence", Number(counts.sourceIntelligenceRuns ?? 0) > 0, { sourceIntelligenceRuns: counts.sourceIntelligenceRuns }),
    check("api-has-dashboard", Number(counts.dashboards ?? 0) > 0 || dashboards.dashboards?.length > 0, { dashboards: counts.dashboards ?? dashboards.dashboards?.length }),
