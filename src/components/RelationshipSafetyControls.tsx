...
export default function RelationshipSafetyControls({ relationshipForm, rightFields, setRelationshipForm }: RelationshipSafetyControlsProps) {
    ...
    <details className="advancedDetails compactAdvanced relationshipTechnicalDetails" data-testid="relationship-safety-controls">
      ...
      <div className="formGrid">
        <label><span>{biText("右表前置过滤字段", "Right pre-filter field")}</span>
          <select value={filter?.field ?? ""} onChange={(event) => setRelationshipForm((current) => ({ ...current, filters: event.target.value ? [{ phase: "pre", side: "right", field: event.target.value, operator: current.filters?.[0]?.operator ?? "eq", value: String(current.filters?.[0]?.value ?? "") }] : [] }))}>
        ...
        <label><span>{biText("过滤条件", "Filter operator")}</span>
          <select disabled={!filter} value={filter?.operator ?? "eq"} onChange={(event) => setRelationshipForm((current) => ({ ...current, filters: current.filters?.length ? [{ ...current.filters[0], operator: event.target.value }] : [] }))}>
        ...
        <label><span>{biText("过滤值", "Filter value")}</span><input disabled={!filter} value={String(filter?.value ?? "")} onChange={(event) => setRelationshipForm((current) => ({ ...current, filters: current.filters?.length ? [{ ...current.filters[0], value: event.target.value }] : [] }))} /></label>
        <label><span>{biText("右表预聚合指标", "Right pre-aggregation measure")}</span>
          <select value={measure?.field ?? ""} onChange={(event) => setRelationshipForm((current) => ({ ...current, preaggregation: event.target.value ? { side: "right", groupFields: rightMappings, measures: [{ field: event.target.value, aggregation: current.preaggregation?.measures?.[0]?.aggregation ?? "sum" }] } : undefined }))}>
        ...
        <label><span>{biText("聚合方式", "Aggregation")}</span>
          <select disabled={!measure} value={measure?.aggregation ?? "sum"} onChange={(event) => setRelationshipForm((current) => current.preaggregation ? ({ ...current, preaggregation: { ...current.preaggregation, measures: current.preaggregation.measures.map((item, index) => index === 0 ? { ...item, aggregation: event.target.value } : item) } }) : current)}>
