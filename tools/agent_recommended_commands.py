...
def build_agent_recommended_commands(
    *,
    ...
    if answer_card.get("query"):
        ...
        recommended.append(
            "python tools/bi_cli.py --json query "
            f"--table {answer_query['table']}{group_arg} --measure {answer_query['measure']} --agg {answer_query['aggregation']}"
        )
    elif selected_table:
        recommended.append(f"python tools/bi_cli.py --json inspect-table --table {selected_table['table_key']}")
    if (not read_only) and intents.wants_import:
        if import_file:
            recommended.append(f"python tools/bi_cli.py --json preview-import {import_file}")
    if (not read_only) and relationship_action:
        ...
        if all([left_table, right_table, left_field, right_field]):
            recommended.append(
                "python tools/bi_cli.py --json relationship-preview "
    ...
    if intents.wants_dashboard and selected_dashboard:
        recommended.append(f"python tools/bi_cli.py --json dashboards --dashboard {selected_dashboard['dashboard_key']}")
    if (not read_only) and intents.wants_dashboard_filter and dashboard_filter_action:
        recommended.append(
            "python tools/bi_cli.py --json add-filter "
    ...
    if (not read_only) and intents.wants_widget and widget_action:
        ...
        recommended.append(
            "python tools/bi_cli.py --json add-widget "
    ...
    if (not read_only) and dashboard_action:
        dashboard_op_command = f"python tools/bi_cli.py --json dashboard-op --op {dashboard_action['op']} --dashboard {dashboard_action['dashboardKey']}"
    ...
    if (not read_only) and intents.wants_index and selected_table and index_field:
        recommended.append(f"python tools/bi_cli.py --json create-index --table {selected_table['table_key']} --field {index_field}")
    if (not read_only) and intents.wants_formula and formula_action:
        recommended.append(
            "python tools/bi_cli.py --json save-formula "
    ...
    if (not read_only) and intents.wants_view and view_action:
        ...
        recommended.append(
            "python tools/bi_cli.py --json save-view "
    ...
    if (not read_only) and intents.wants_metric and metric_action:
        ...
        recommended.append(
            "python tools/bi_cli.py --json add-metric "
    ...
    if (not read_only) and intents.wants_semantic and semantic_action:
        recommended.append(
            "python tools/bi_cli.py --json set-semantic "
