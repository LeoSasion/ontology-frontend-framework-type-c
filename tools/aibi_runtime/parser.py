...
def build_parser() -> argparse.ArgumentParser:
    ...
    workflow_recipe_preview = sub.add_parser("workflow-recipe-preview", description="Preview a bounded reusable Workflow Recipe.")
    workflow_recipe_preview.add_argument("--workspace", default="")
    workflow_recipe_preview.add_argument("--request-key", required=True)
    workflow_recipe_preview.add_argument("--name", required=True)
    workflow_recipe_preview.add_argument("--description", default="")
    workflow_recipe_preview.add_argument("--stage-json", action="append", default=[])

    workflow_recipe_publish = sub.add_parser("workflow-recipe-publish", description="Publish one exact reusable Workflow Recipe.")
    workflow_recipe_publish.add_argument("--workspace", default="")
    workflow_recipe_publish.add_argument("--request-key", required=True)
    workflow_recipe_publish.add_argument("--name", required=True)
    workflow_recipe_publish.add_argument("--description", default="")
    workflow_recipe_publish.add_argument("--stage-json", action="append", default=[])
    workflow_recipe_publish.add_argument("--expected-plan", required=True)
    workflow_recipe_publish.add_argument("--yes", action="store_true")

    workflow_recipes = sub.add_parser("workflow-recipes", description="List or inspect workspace-scoped Workflow Recipes.")
    workflow_recipes.add_argument("--workspace", default="")
    workflow_recipes.add_argument("--recipe", default="")
    workflow_recipes.add_argument("--limit", type=int, default=50)

    workflow_recipe_plan = sub.add_parser("workflow-recipe-plan", description="Instantiate a Workflow Recipe as a non-executing capability plan.")
    workflow_recipe_plan.add_argument("--workspace", default="")
    workflow_recipe_plan.add_argument("--recipe", required=True)
    workflow_recipe_plan.add_argument("--bindings-json", default="{}")
    ...
    semantic_release_preview = sub.add_parser("semantic-release-preview", description="Build a read-only atomic release plan for current semantic patch proposals.")
    semantic_release_preview.add_argument("--workspace", default="")
    semantic_release_preview.add_argument("--request-key", required=True)
    semantic_release_preview.add_argument("--label", default="Semantic release")
    semantic_release_preview.add_argument("--proposal", action="append", default=[])

    semantic_release_publish = sub.add_parser("semantic-release-publish", description="Publish one exact reviewed semantic release atomically.")
    semantic_release_publish.add_argument("--workspace", default="")
    semantic_release_publish.add_argument("--request-key", required=True)
    semantic_release_publish.add_argument("--expected-plan", required=True)
    semantic_release_publish.add_argument("--label", default="Semantic release")
    semantic_release_publish.add_argument("--proposal", action="append", default=[])
    semantic_release_publish.add_argument("--yes", action="store_true")

    semantic_releases = sub.add_parser("semantic-releases", description="List or inspect workspace-scoped semantic releases with live drift checks.")
    semantic_releases.add_argument("--workspace", default="")
    semantic_releases.add_argument("--release", default="")
    semantic_releases.add_argument("--limit", type=int, default=50)

    semantic_release_rollback = sub.add_parser("semantic-release-rollback", description="Preview or confirm rollback of one current semantic release.")
    semantic_release_rollback.add_argument("--workspace", default="")
    semantic_release_rollback.add_argument("--release", required=True)
    semantic_release_rollback.add_argument("--request-key", required=True)
    semantic_release_rollback.add_argument("--expected-plan", default="")
    semantic_release_rollback.add_argument("--yes", action="store_true")
    ...
    export_analysis = sub.add_parser("export-analysis", description="Export one receipt-bound Analysis Unit as deterministic Excel, Word, PowerPoint, or report artifacts without requerying.")
    ...
    export_analysis.add_argument("--format", action="append", choices=["xlsx", "docx", "pptx", "md"], default=[])
    ...
    research_runs = sub.add_parser("research-runs", description="List or inspect finite workspace Research Runs with live Anchor freshness and immutable revisions.")
    research_runs.add_argument("--research", default="")
    research_runs.add_argument("--limit", type=int, default=30)

    research_run_create = sub.add_parser("research-run-create", description="Preview or confirm a finite Research Run from one current Exploration Anchor.")
    research_run_create.add_argument("--thread", required=True)
    research_run_create.add_argument("--anchor", default="")
    research_run_create.add_argument("--goal", required=True)
    research_run_create.add_argument("--skill", default="")
    research_run_create.add_argument("--hypothesis", action="append", default=[])
    research_run_create.add_argument("--counterexample", action="append", default=[])
    research_run_create.add_argument("--sensitivity", action="append", default=[])
    research_run_create.add_argument("--max-observations", type=int, default=8)
    research_run_create.add_argument("--max-revisions", type=int, default=3)
    research_run_create.add_argument("--expected-plan", default="")
    research_run_create.add_argument("--yes", action="store_true")

    research_run_revise = sub.add_parser("research-run-revise", description="Preview or append one immutable Research Plan revision.")
    research_run_revise.add_argument("--research", required=True)
    research_run_revise.add_argument("--reason", required=True)
    research_run_revise.add_argument("--goal", default="")
    research_run_revise.add_argument("--skill", default=None)
    research_run_revise.add_argument("--hypothesis", action="append", default=None)
    research_run_revise.add_argument("--counterexample", action="append", default=None)
    research_run_revise.add_argument("--sensitivity", action="append", default=None)
    research_run_revise.add_argument("--clear-hypotheses", action="store_true")
    research_run_revise.add_argument("--clear-counterexamples", action="store_true")
    research_run_revise.add_argument("--clear-sensitivities", action="store_true")
    research_run_revise.add_argument("--expected-revision", required=True)
    research_run_revise.add_argument("--expected-plan", default="")
    research_run_revise.add_argument("--yes", action="store_true")

    research_run_observe = sub.add_parser("research-run-observe", description="Preview or adopt one current same-thread Anchor as bounded research evidence.")
    research_run_observe.add_argument("--research", required=True)
    research_run_observe.add_argument("--anchor", required=True)
    research_run_observe.add_argument("--kind", required=True, choices=["evidence", "counterexample", "sensitivity"])
    research_run_observe.add_argument("--step", required=True)
    research_run_observe.add_argument("--verdict", required=True, choices=["supports", "challenges", "inconclusive"])
    research_run_observe.add_argument("--note", required=True)
    research_run_observe.add_argument("--expected-revision", required=True)
    research_run_observe.add_argument("--expected-plan", default="")
    research_run_observe.add_argument("--yes", action="store_true")

    research_run_finalize = sub.add_parser("research-run-finalize", description="Preview or complete a finite Research Run with explicit counterexample and sensitivity coverage.")
    research_run_finalize.add_argument("--research", required=True)
    research_run_finalize.add_argument("--expected-revision", required=True)
    research_run_finalize.add_argument("--expected-plan", default="")
    research_run_finalize.add_argument("--yes", action="store_true")
    ...
    metric_monitors = sub.add_parser("metric-monitors", description="List or inspect workspace-scoped Metric Monitor definitions and replayable local evaluations.")
    metric_monitors.add_argument("--monitor", default="")
    metric_monitors.add_argument("--status", default="", choices=["", "active", "replaced", "deleted"])
    metric_monitors.add_argument("--limit", type=int, default=30)

    metric_monitor_create = sub.add_parser("metric-monitor-create", description="Preview or confirm a local Metric Monitor definition bound to one current scalar Snapshot.")
    metric_monitor_create.add_argument("--snapshot", required=True)
    metric_monitor_create.add_argument("--label", required=True)
    metric_monitor_create.add_argument("--metric", default="")
    metric_monitor_create.add_argument("--cadence", default="manual", choices=["manual", "daily", "weekly", "monthly"])
    metric_monitor_create.add_argument("--strategy", default="absolute-change", choices=["absolute-change", "percent-change"])
    metric_monitor_create.add_argument("--direction", default="absolute", choices=["increase", "decrease", "absolute"])
    metric_monitor_create.add_argument("--threshold", type=float)
    metric_monitor_create.add_argument("--warning-ratio", type=float, default=0.8)
    metric_monitor_create.add_argument("--expected-plan", default="")
    metric_monitor_create.add_argument("--yes", action="store_true")

    metric_monitor_replace = sub.add_parser("metric-monitor-replace", description="Preview or confirm an immutable replacement Metric Monitor definition.")
    metric_monitor_replace.add_argument("--monitor", required=True)
    metric_monitor_replace.add_argument("--snapshot", required=True)
    metric_monitor_replace.add_argument("--label", required=True)
    metric_monitor_replace.add_argument("--metric", default="")
    metric_monitor_replace.add_argument("--cadence", default="manual", choices=["manual", "daily", "weekly", "monthly"])
    metric_monitor_replace.add_argument("--strategy", default="absolute-change", choices=["absolute-change", "percent-change"])
    metric_monitor_replace.add_argument("--direction", default="absolute", choices=["increase", "decrease", "absolute"])
    metric_monitor_replace.add_argument("--threshold", type=float)
    metric_monitor_replace.add_argument("--warning-ratio", type=float, default=0.8)
    metric_monitor_replace.add_argument("--expected-plan", default="")
    metric_monitor_replace.add_argument("--yes", action="store_true")

    metric_monitor_delete = sub.add_parser("metric-monitor-delete", description="Preview or confirm retiring a Metric Monitor definition while preserving its evaluation Trace.")
    metric_monitor_delete.add_argument("--monitor", required=True)
    metric_monitor_delete.add_argument("--expected-plan", default="")
    metric_monitor_delete.add_argument("--yes", action="store_true")

    metric_monitor_run = sub.add_parser("metric-monitor-run", description="Run one bounded local Metric Monitor evaluation without notification, scheduling, or business writes.")
    metric_monitor_run.add_argument("--monitor", required=True)
    metric_monitor_run.add_argument("--snapshot", default="")
    ...
    decision_frameworks = sub.add_parser("decision-frameworks", description="List or inspect workspace decision frameworks with live evidence freshness.")
    decision_frameworks.add_argument("--framework", default="")
    decision_frameworks.add_argument("--unit", default="")
    decision_frameworks.add_argument("--limit", type=int, default=30)

    decision_framework_create = sub.add_parser("decision-framework-create", description="Create an empty SWOT or process draft from one current Analysis Unit.")
    decision_framework_create.add_argument("--unit", required=True)
    decision_framework_create.add_argument("--framework-type", required=True, choices=["swot", "process"])
    decision_framework_create.add_argument("--title", required=True)
    decision_framework_create.add_argument("--request-key", required=True)

    decision_framework_save = sub.add_parser("decision-framework-save", description="Save explicitly typed framework claims against the current content fingerprint.")
    decision_framework_save.add_argument("--framework", required=True)
    decision_framework_save.add_argument("--title", required=True)
    decision_framework_save.add_argument("--claims-json", required=True)
    decision_framework_save.add_argument("--expected-content", required=True)

    decision_framework_publish = sub.add_parser("decision-framework-publish", description="Preview or confirm one reviewed decision-framework publication.")
    decision_framework_publish.add_argument("--framework", required=True)
    decision_framework_publish.add_argument("--yes", action="store_true")
    decision_framework_publish.add_argument("--expected-plan", default="")

    decision_framework_export = sub.add_parser("decision-framework-export", description="Export a safe current or stale-unloaded decision framework.")
    decision_framework_export.add_argument("--framework", required=True)

    reviewed_publication_plan = sub.add_parser("reviewed-publication-plan", description="Preview a reviewed evidence publication without writing it.")
    reviewed_publication_plan.add_argument("--memory", required=True)
    reviewed_publication_plan.add_argument("--unit", required=True)
    reviewed_publication_plan.add_argument("--title", required=True)
    reviewed_publication_plan.add_argument("--content-json", required=True)
    reviewed_publication_plan.add_argument("--skill-fingerprint", default="")

    reviewed_publication_publish = sub.add_parser("reviewed-publication-publish", description="Preview or confirm one reviewed evidence publication.")
    reviewed_publication_publish.add_argument("--memory", required=True)
    reviewed_publication_publish.add_argument("--unit", required=True)
    reviewed_publication_publish.add_argument("--title", required=True)
    reviewed_publication_publish.add_argument("--content-json", required=True)
    reviewed_publication_publish.add_argument("--skill-fingerprint", default="")
    reviewed_publication_publish.add_argument("--expected-plan", default="")
    reviewed_publication_publish.add_argument("--yes", action="store_true")

    reviewed_publications = sub.add_parser("reviewed-publications", description="List or inspect current reviewed evidence publications.")
    reviewed_publications.add_argument("--publication", default="")
    reviewed_publications.add_argument("--status", default="", choices=["", "current", "drifted", "deprecated", "integrity_failed"])
    reviewed_publications.add_argument("--skill-fingerprint", default="")
    reviewed_publications.add_argument("--limit", type=int, default=100)

    reviewed_publication_deprecate = sub.add_parser("reviewed-publication-deprecate", description="Preview or confirm append-only deprecation of a reviewed publication.")
    reviewed_publication_deprecate.add_argument("--publication", required=True)
    reviewed_publication_deprecate.add_argument("--reason", required=True)
    reviewed_publication_deprecate.add_argument("--expected-head", default="")
    reviewed_publication_deprecate.add_argument("--yes", action="store_true")

    reviewed_publication_export = sub.add_parser("reviewed-publication-export", description="Export a safe current or stale-unloaded reviewed publication.")
    reviewed_publication_export.add_argument("--publication", required=True)
    reviewed_publication_export.add_argument("--skill-fingerprint", default="")
    reviewed_publication_export.add_argument("--forensic", action="store_true")

    sub.add_parser("evidence-retrieval-status", description="Report the approved hybrid-retrieval mode without enabling a provider.")
    evidence_retrieval_receipts = sub.add_parser("evidence-retrieval-receipts", description="List redacted evidence-retrieval receipts.")
    evidence_retrieval_receipts.add_argument("--limit", type=int, default=100)
    evidence_retrieval_evaluate = sub.add_parser("evidence-retrieval-evaluate", description="Run a fixed offline provider evaluation; never self-enable embeddings.")
    evidence_retrieval_evaluate.add_argument("--provider-profile", default="")
    ...
    import_job_create = sub.add_parser("import-job-create", description="Create a plan-bound durable import job.")
    import_job_create.add_argument("--import-kind", default="single", choices=["single", "folder"])
    import_job_create.add_argument("--path", required=True)
    import_job_create.add_argument("--request-key", required=True)
    import_job_create.add_argument("--expected-plan", required=True)
    import_job_create.add_argument("--stage-key", default="")
    import_job_create.add_argument("--stage-bindings", default="")
    import_job_create.add_argument("--workspace", default="")
    import_job_create.add_argument("--label", default="")
    import_job_create.add_argument("--table")
    import_job_create.add_argument("--name")
    import_job_create.add_argument("--mode", default="create", choices=["create", "merge", "replace"])
    import_job_create.add_argument("--confirm-schema-change", action="store_true")
    import_job_create.add_argument("--unique-fields")
    import_job_create.add_argument("--conflict-rule", choices=["overwrite", "fill-empty", "skip-existing"])
    import_job_create.add_argument("--limit", type=int, default=200)
    import_job_create.add_argument("--no-recursive", action="store_true")

    import_job_run = sub.add_parser("import-job-run", description="Run one queued durable import job.")
    import_job_run.add_argument("--job", required=True)
    import_job_run.add_argument("--workspace", default="")
    import_job_run.add_argument("--lease-token", default="")

    import_job_resume = sub.add_parser("import-job-resume", description="Resume a reconciled import job after its frozen plan is revalidated.")
    import_job_resume.add_argument("--job", required=True)
    import_job_resume.add_argument("--workspace", default="")

    import_job_recover = sub.add_parser("import-job-recover", description="Reconcile unfinished import activations after runtime restart.")
    import_job_recover.add_argument("--workspace", default="")
    import_job_recover.add_argument("--all", action="store_true")

    import_job_process_exit = sub.add_parser("import-job-process-exit", description="Reconcile an owned import worker exit.")
    import_job_process_exit.add_argument("--job", required=True)
    import_job_process_exit.add_argument("--workspace", default="")
    import_job_process_exit.add_argument("--exit-code", type=int)
    import_job_process_exit.add_argument("--signal", default="")
    import_job_process_exit.add_argument("--lease-token", required=True)

    sqlserver_probe = sub.add_parser("sqlserver-adapter-probe", description="Inspect the optional SQL Server snapshot capability without connecting.")
    sqlserver_probe.add_argument("--connector", default="")
    sqlserver_test = sub.add_parser("sqlserver-adapter-test", description="Test one allowlisted SQL Server target with read-only credentials.")
    sqlserver_test.add_argument("--connector", required=True)
    sqlserver_test.add_argument("--timeout", type=int, default=10)
    sqlserver_discover = sub.add_parser("sqlserver-adapter-discover", description="Read and persist a bounded SQL Server catalog.")
    sqlserver_discover.add_argument("--connector", required=True)
    sqlserver_discover.add_argument("--max-tables", type=int, default=8)
    sqlserver_discover.add_argument("--max-columns", type=int, default=64)
    sqlserver_discover.add_argument("--timeout", type=int, default=30)
    sqlserver_preview = sub.add_parser("sqlserver-adapter-preview", description="Read bounded statistics only for reviewed SQL Server resources.")
    sqlserver_preview.add_argument("--connector", required=True)
    sqlserver_preview.add_argument("--catalog", required=True)
    sqlserver_preview.add_argument("--resources", required=True)
    sqlserver_preview.add_argument("--sample-rows", type=int, default=200)
    sqlserver_preview.add_argument("--timeout", type=int, default=30)
    sqlserver_plan = sub.add_parser("sqlserver-adapter-plan", description="Persist an idempotent, bounded SQL Server snapshot plan.")
    sqlserver_plan.add_argument("--connector", required=True)
    sqlserver_plan.add_argument("--request-key", required=True)
    sqlserver_plan.add_argument("--catalog", required=True)
    sqlserver_plan.add_argument("--selections-json", required=True)
    sqlserver_plan.add_argument("--budget-json", default="{}")
    sqlserver_snapshot = sub.add_parser("sqlserver-adapter-snapshot", description="Confirm one reviewed SQL Server plan into isolated staging.")
    sqlserver_snapshot.add_argument("--connector", required=True)
    sqlserver_snapshot.add_argument("--request-key", required=True)
    sqlserver_snapshot.add_argument("--expected-plan", required=True)
    sqlserver_snapshot.add_argument("--yes", action="store_true")
    sqlserver_activate = sub.add_parser("sqlserver-adapter-activate", description="Queue one sealed SQL Server snapshot through the standard durable import runtime.")
    sqlserver_activate.add_argument("--connector", required=True)
    sqlserver_activate.add_argument("--workspace", default="")
    sqlserver_activate.add_argument("--request-key", required=True)
    sqlserver_activate.add_argument("--expected-plan", required=True)
    sqlserver_activate.add_argument("--expected-manifest", required=True)
    sqlserver_activate.add_argument("--yes", action="store_true")
    sqlserver_activation_status = sub.add_parser("sqlserver-adapter-activation-status", description="Verify one SQL Server activation against its durable import and finalized Journal.")
    sqlserver_activation_status.add_argument("--connector", required=True)
    sqlserver_activation_status.add_argument("--workspace", default="")
    sqlserver_activation_status.add_argument("--request-key", required=True)
    sqlserver_activation_status.add_argument("--expected-plan", required=True)
    sqlserver_activation_status.add_argument("--expected-manifest", required=True)
    sqlserver_activation_finalize = sub.add_parser("sqlserver-adapter-activation-finalize", description="Persist active only after a SQL Server durable import Journal finalized as committed.")
    sqlserver_activation_finalize.add_argument("--job", default="")
    sqlserver_activation_finalize.add_argument("--workspace", default="")
    sqlserver_activation_finalize.add_argument("--all", action="store_true")
    sqlserver_activation_finalize.add_argument("--yes", action="store_true")
    ...
    workspace_delete.add_argument("--request-key", default="")
    workspace_delete.add_argument("--expected-plan", default="")
    workspace_delete.add_argument("--yes", action="store_true")

    workspace_recovery_list = sub.add_parser("workspace-recovery-list", description="List bounded recovery points for the active workspace.")
    workspace_recovery_list.add_argument("--workspace", default="")
    workspace_recovery_list.add_argument("--limit", type=int, default=5)
    workspace_recovery_list.add_argument("--verify", action="store_true")

    workspace_recovery_inspect = sub.add_parser("workspace-recovery-inspect", description="Inspect and verify one active-workspace recovery point.")
    workspace_recovery_inspect.add_argument("--workspace", default="")
    workspace_recovery_inspect.add_argument("--recovery-point", required=True)

    workspace_recovery_compare = sub.add_parser("workspace-recovery-compare", description="Compare current table versions with one verified recovery point without exposing rows.")
    workspace_recovery_compare.add_argument("--workspace", default="")
    workspace_recovery_compare.add_argument("--recovery-point", required=True)

    workspace_recovery_reconcile = sub.add_parser("workspace-recovery-reconcile", description="Reconcile unfinished restore journals after a local runtime restart.")
    workspace_recovery_reconcile.add_argument("--all", action="store_true")

    for command, description in (
        ("workspace-recovery-create", "Create a verified active-workspace recovery point."),
        ("workspace-recovery-restore", "Restore one verified point into its original active workspace."),
        ("workspace-recovery-delete", "Delete one verified active-workspace recovery point."),
    ):
        recovery_parser = sub.add_parser(command, description=description)
        recovery_parser.add_argument("--workspace", default="")
        if command == "workspace-recovery-create":
            recovery_parser.add_argument("--reason", required=True)
        else:
            recovery_parser.add_argument("--recovery-point", required=True)
        recovery_parser.add_argument("--request-key", required=True)
        recovery_parser.add_argument("--expected-plan", default="")
        recovery_parser.add_argument("--yes", action="store_true")
    ...
    preview.add_argument("--mode", default="auto", choices=["auto", "create", "merge", "replace"])
    ...
    commit.add_argument("--confirm-schema-change", action="store_true")
    ...
    save_connector.add_argument("--port", type=int, default=1433)
    save_connector.add_argument("--encryption", default="required", choices=["required"])
    ...
    metric_contract_preview = sub.add_parser("metric-contract-preview", description="Preview a versioned Metric Contract v2 with replay baselines.")
    metric_contract_preview.add_argument("--workspace", default="")
    metric_contract_preview.add_argument("--metric", required=True)
    metric_contract_preview.add_argument("--request-key", required=True)
    metric_contract_preview.add_argument("--label", default="")
    metric_contract_preview.add_argument("--population", default="")
    metric_contract_preview.add_argument("--grain", default="")
    metric_contract_preview.add_argument("--unit", default="")
    metric_contract_preview.add_argument("--null-policy", default="exclude", choices=["exclude", "zero", "error"])
    metric_contract_preview.add_argument("--dedup-key", default="")
    metric_contract_preview.add_argument("--direction", default="neutral", choices=["neutral", "increase", "decrease"])
    metric_contract_preview.add_argument("--owner", default="workspace-owner")
    metric_contract_preview.add_argument("--scenario-json", action="append", default=[])

    metric_contract_publish = sub.add_parser("metric-contract-publish", description="Publish one exact Metric Contract v2 after preview.")
    metric_contract_publish.add_argument("--workspace", default="")
    metric_contract_publish.add_argument("--metric", required=True)
    metric_contract_publish.add_argument("--request-key", required=True)
    metric_contract_publish.add_argument("--expected-plan", required=True)
    metric_contract_publish.add_argument("--label", default="")
    metric_contract_publish.add_argument("--population", default="")
    metric_contract_publish.add_argument("--grain", default="")
    metric_contract_publish.add_argument("--unit", default="")
    metric_contract_publish.add_argument("--null-policy", default="exclude", choices=["exclude", "zero", "error"])
    metric_contract_publish.add_argument("--dedup-key", default="")
    metric_contract_publish.add_argument("--direction", default="neutral", choices=["neutral", "increase", "decrease"])
    metric_contract_publish.add_argument("--owner", default="workspace-owner")
    metric_contract_publish.add_argument("--scenario-json", action="append", default=[])
    metric_contract_publish.add_argument("--yes", action="store_true")

    metric_contracts = sub.add_parser("metric-contracts", description="List or inspect versioned Metric Contracts with freshness.")
    metric_contracts.add_argument("--workspace", default="")
    metric_contracts.add_argument("--metric", default="")
    metric_contracts.add_argument("--contract", default="")
    metric_contracts.add_argument("--limit", type=int, default=50)

    metric_contract_replay = sub.add_parser("metric-contract-replay", description="Replay a published Metric Contract against the current validated replica.")
    metric_contract_replay.add_argument("--workspace", default="")
    metric_contract_replay.add_argument("--contract", required=True)
