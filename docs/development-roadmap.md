...
当前没有已批准但尚未交付的主项。P2 复杂跨表已经进入稳定受控能力；新的优先级必须先写明用户结果、边界和退出条件，再进入本队列。
...
A-F 主链和 P2 复杂跨表已经交付：业务意图、上下文、证据计划、事件、声明式 Skill、固定 Policy Hook、可恢复 Session、Fork、证据感知快照、受控 Provider Registry、出站门禁、评估回执、固定工作流图、只读专家视图、Join 完整性校验，以及一至三跳线性执行与逐跳证明均已落地。
