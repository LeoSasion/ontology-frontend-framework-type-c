...
ERP 路径是证据驱动的可选 Domain Pack，不是固定模板。只有工作区显式启用后，系统才根据当前表字段选择指标、图表、表格、筛选器和证据单元；缺少必需字段的单元必须省略并解释。
...
1. 读取当前工作区表注册和字段语义。
2. 验证 ERP Pack 已在当前工作区启用；未启用时返回不可用而不是自动匹配。
3. 用别名组和业务信号匹配角色。
4. 删除缺少必需指标或维度的单元。
5. 按必需字段、可选字段和领域锚点评分。
6. 在请求上限内选择证据最充分的单元。
7. 返回命中、类别覆盖、省略原因和下一步所需字段。
8. 在每个组件 payload 保留 Pack 版本、单元 key、匹配字段、来源标识和证据引用。
