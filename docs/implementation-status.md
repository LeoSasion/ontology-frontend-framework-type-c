...
| 本地运维 | 稳定 | SQLite schema v9、DuckDB schema v1；Agent Session、Turn、事件、Context Snapshot、Skill、Runtime Profile、Knowledge Source、Semantic Patch、Confirmed Plan Memory 与 Recall Receipt 已版本化；兼容检查、配置可移植、隔离迁移、恢复点和双库回滚可用；`preflight` 只停止本轮拥有且令牌验证通过的服务，陈旧 PID 不会直接触发终止 |
