...
当前阶段：single-user and local-only 的通用可信分析基线。M0–M11 与 Core / Domain Pack / Connector Adapter 解耦已进入当前代码基线；新工作区默认不启用任何领域知识。最新归档发布验收见 [2026-07-14 发布回执](../artifacts/release-2026-07-14/SUMMARY.md)，当前代码于 2026-07-15 通过完整 `npm run preflight`。
...
| Domain runtime | 工作区级 Domain Pack 注册表 | 空默认；显式预演和确认后才启停；历史证据不原地重解释 |
| Connector runtime | 统一 Adapter 能力目录 | 只有声明为 available 的 Adapter 可操作；接入能力不授予领域语义 |
...
| Generic domain extension framework | Stable initial | Manifest、兼容性校验、工作区启停、前后端统一目录、运行上下文和 Pack 指纹已闭环；平台电商与 ERP 能力仅在对应 Pack 显式启用后注入。 |
...
- 当前两个内置 Domain Pack 随 AIBI-C 代码发布但默认停用；尚不支持从外部安装、签名、卸载或独立升级 Pack。
