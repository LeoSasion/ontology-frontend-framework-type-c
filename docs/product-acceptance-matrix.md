...
| Create one chart | The default dashboard action is one guided chart. | AI asks at most one necessary question when fields are unclear, shows clickable candidate fields, drafts one chart only when fields are explicit enough, and stops before write confirmation. |
...
| Desktop visual ratios | The Views page stays usable across common PC shapes. | Live browser screenshots pass at 1440x900, 900x1440, and 1100x1100 with no global overflow, visible overlap, or clipped text. |
...
| Temporary real import loop | A real file or folder can complete the beginner path without touching the user's active workspace. | UI verification creates a temporary workspace, imports a real local folder when present or a real file as fallback, generates evidence, opens a dashboard, requests one chart, opens evidence, restores the original workspace, and deletes the temporary workspace. |
